...
void *pthread_sen()
{
    ...
    while(1)
    {
        ...
        if (0 < recv(sockfd, &F, sizeof(F), MSG_DONTWAIT));
            ...
            //刷白网络连接判断标识
            lcd_draw_character(R_x + 50, L_1_y + 150, L"NO",  0xFFFFFF);
            lcd_draw_character(R_x + 50, L_1_y + 150, L"YES",  0xFFFFFF);
        //if (getsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE, &flag, (unsigned int *)sizeof(int)) < 0);
        if ('O' != F[0])
        {
            //无连接
            
            lcd_draw_character(R_x + 50, L_1_y + 150, L"NO",  0x9900FF);
            printf("########################################  NO\n");
            F[0] = '1';
        }
        if ('O' == F[0])
        {
             lcd_draw_character(R_x + 50, L_1_y + 150, L"YES",  0x9900FF);
             printf("########################################  Y\n");
             F[0] = '1';
        }
